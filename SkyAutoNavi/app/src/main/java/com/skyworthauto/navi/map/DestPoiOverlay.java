...
public class DestPoiOverlay {
    ...
    public void zoomToSpan() {
        if (mPois != null && mPois.size() > 0) {
            ...
            mamap.moveCamera(CameraUpdateFactory.newLatLngBoundsRect(bounds, 400, 100, 100, 100));
            mamap.moveCamera(CameraUpdateFactory.changeLatLng(
                    new LatLng(mPois.get(0).getLatLonPoint().getLatitude(),
                            mPois.get(0).getLatLonPoint().getLongitude())));
        }
    }
    ...
    protected BitmapDescriptor getBitmapDescriptor(int index) {
        if (index >= 0 && index < MAX_INDEX) {
            BitmapDescriptor icon = BitmapDescriptorFactory
                    .fromBitmap(BitmapFactory.decodeResource(Utils.getResources(), MARKERS[index]));
            return icon;
        } else {
            BitmapDescriptor icon = BitmapDescriptorFactory.fromBitmap(BitmapFactory
                    .decodeResource(Utils.getResources(), R.drawable.marker_other_highlight));
            return icon;
        }
    }
        ...
        if (marker.getObject() != null) {
            try {
                ...
                marker.setIcon(BitmapDescriptorFactory.fromBitmap(
                        BitmapFactory.decodeResource(Utils.getResources(), MARKERS_HL[index])));
                marker.setToTop();
            } catch (Exception e) {
            }
        } else {
    ...
    private void resetlastmarker() {
        ...
        if (index >= 0 && index < MAX_INDEX) {
            mlastMarker.setIcon(BitmapDescriptorFactory.fromBitmap(
                    BitmapFactory.decodeResource(Utils.getResources(), MARKERS[index])));
        } else {
